...
CellTrekFeaturePlot = function(celltrek_obj, feature, pt_size = 0.7, color_max = 0.85, # set max to 85%
                               plot_title ='',
                               # Edges
                               show_edge =F, plot_info_obj, segment_size = 0.2 ){
    # Empty plot
    p = ggplot()
    # Edge
    if(show_edge){
        if(missing(plot_info_obj)) stop('When show_edge = T, need the plot_info_obj. Please use PrepareSpatialPlotObject to get this object')
    ...
    # Plot
    p = p + 
        geom_point(data= expr_coord_table, aes(y = -coord_x, x = coord_y, color = .data[[feature]]),
                  size = pt_size) + 
...
# # PLot expression 
# CellTrekSpatialFeaturePlot = function(celltrek_obj, feature, pt_size = 0.7, color_max = 0.85, # set max to 85%
#                                plot_title ='',
#                                # Edges
